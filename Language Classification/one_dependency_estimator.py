...
class AODE():
    ...
    def partial_fit(self, dataset):
        ...
        for idx, i in aux_dataset.iterrows():
            ...
            newIndex = np.array(
                list(map(lambda x: x + self.features_index1 + X, self.features_index2 + X * self.features_values))[
                :]).ravel()
        ...
        for idx, i in dataset.dtypes.iteritems():
            if (i != 'category') | (dataset[idx].cat.categories.tolist() != self.variables_dict[idx].tolist()):
                raise AttributeError(
                    "attribute error: {} is not a category variable or its categories are wrong".format(idx))
